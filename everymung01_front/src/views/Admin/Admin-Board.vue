  ...
  methods: {
    ...
    delet(status){
      ...
      if(status === false) {
        ...
        this.board.status=false//영주
        this.$http.post(`http://localhost:1234/deleteBoardByMngr`,this.board)
              .then(res =>{
                  const idx = this.rows.findIndex(x => x.boardNo === boardNo)
                  //const idx2 = this.rows.findIndex(x => x.groupNo === groupNo) //영주
                  this.dialog=false
                  this.rows.splice(idx, 1)
                  //this.rows.splice(idx2, 1) //영주 
                  history.go(0)//영주_자동 재조회 
                }).catch(err =>{
                  alert(err+"\n"+"Admin-Board(delet) 에러")
                })
    ...
    update(content){
      ...
      this.$http.post(`http://localhost:1234/updateBoardByMngr`,this.board)//수정-에러 발생_호출컨트롤러 변경해보겠음
              .then(res =>{
                  this.selectAll();
                  this.content=null
                }).catch(err =>{
